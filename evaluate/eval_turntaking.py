...
# The latency of interruptions is not included in the average; only the count is tracked.
...
def eval_smooth_turn_taking(data_dir):
    ...
    print("take_turn_list:", len(take_turn_list))
    average_latency = sum(latency_list) / len(latency_list) if len(latency_list) > 0 else 0
